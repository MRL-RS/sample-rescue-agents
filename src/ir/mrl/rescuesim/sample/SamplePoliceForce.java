...
//            sendClear(time, target.getX(), target.getY());
    ...
    private List<Integer> getRandomBuildings() {
        return getRandomInEntities(buildings);
    }


    private List<Integer> getRandomInEntities(List<StandardEntity> entities) {
        List<Integer> randomRoads = new ArrayList<>();


        int bound = random.nextInt(entities.size() / 2) + 10;
        for (int i = 0; i < bound; i++) {
            int index = random.nextInt(entities.size());
            randomRoads.add(entities.get(index).getID().getValue());
        }
        return randomRoads;
    }
